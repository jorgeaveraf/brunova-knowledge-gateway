...
from app.adapters.google_workspace.auth import (
    ...
    build_keyless_signing_credentials,
)
...
def test_build_keyless_signing_credentials_uses_adc_remote_signer():
    settings = Settings(
        workspace_delegated_user="reader@example.com",
        workspace_service_account_email="gateway@project.iam.gserviceaccount.com",
        workspace_doc_max_chars=10000,
    ...
    with (
        ...
        signing = build_keyless_signing_credentials(settings)
    ...
    assert signing.service_account_email == settings.workspace_service_account_email
    assert signing.scopes == (CLOUD_PLATFORM_SCOPE,)
    assert signing._subject is None
